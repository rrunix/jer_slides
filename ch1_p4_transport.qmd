...
- Primer envío: seq=100 con "Hola" (4 bytes) → el receptor responde ACK=104, indicando que espera el byte 104
- Envíos consecutivos: Cada mensaje usa el número de secuencia donde terminó el anterior (104, 111, 115)
- ACK acumulativo final: ACK=121 confirma de una vez ambos paquetes "TCP" y "rocks"
...
- Cada extremo debe enviar su propio FIN y recibir confirmación, permitiendo cierre unidireccional (half-close).
- El cliente espera 2*MSL para asegurar que su último ACK llegó y manejar retransmisiones tardías antes del cierre definitivo.
:::
:::
